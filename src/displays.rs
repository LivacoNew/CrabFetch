...
impl Default for DisplayConfiguration {
    fn default() -> Self {
        DisplayConfiguration {
            ...
            title_italic: None,
            seperator: None,
            format: "{width}x{height}".to_string()
        }
    }
}
impl ModuleConfiguration for DisplayConfiguration {
    fn apply_toml_line(&mut self, key: &str, value: &str) -> Result<(), crate::config_manager::TOMLParseError> {
        match key {
            ...
            _ => return Err(TOMLParseError::new("Unknown key.".to_string(), Some("Displays".to_string()), Some(key.to_string()), value.to_string()))
        }
        Ok(())
    }
}
