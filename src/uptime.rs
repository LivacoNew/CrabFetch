...
impl ModuleConfiguration for UptimeConfiguration {
    fn apply_toml_line(&mut self, key: &str, value: &str) -> Result<(), crate::config_manager::TOMLParseError> {
        match key {
            ...
            _ => return Err(TOMLParseError::new("Unknown key.".to_string(), Some("Uptime".to_string()), Some(key.to_string()), value.to_string()))
        }
        Ok(())
    }
}
